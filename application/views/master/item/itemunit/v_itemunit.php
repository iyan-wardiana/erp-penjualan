					...
					<div class="col-xl-12">
						<div class="card">
                                    ...
                                    <form>
										<div class="row">
                                            <div class="mb-3 col-md-4">
                                                ...
                                                <input type="text" name="UNIT_NAME" id="UNIT_NAME" class="form-control" value="" placeholder="Unit Name...">
                                            </div>
                                        </div>
                                    </form>
                                </div>
							...
							<div class="card-footer d-sm-flex justify-content-between">
								...
								<a href="<?=$url_Add?>" class="btn btn-primary btn-sm">+ Add Unit</a>
							</div>
						</div>
					</div>
				</div>
									...
									<table id="dataTable" class="table table-hover table-responsive-sm bordered" width="100%">
										<thead class="thead-primary">
											<tr>
												...
												<th width="100" style="text-align: center;">Unit Code</th>
			...
			if(data.length > 0) 
			{
				const table 	= $('#dataTable').DataTable({
					...
					columns: [
						...
						{ data: 'UNIT_CODE', className: 'dt-body-center' },
						{ data: 'UNIT_NAME'},
						{ data: 'UNIT_DESC'},
						{ data: 'Action', className: 'dt-body-center', orderable: false,
							render: function(data, type, row, meta) {
								return '<a href="<?php echo site_url('itemunit/update/') ?>'+row['no']+'" class="btn btn-info btn-icon-xs me-2"><i class="fa-solid fa-pen-to-square"></i></a>'+
		...
		function deleteData(docNum)
		{
			...
			swalWithBootstrapButtons.fire({
				...
				if (result.isConfirmed) {
					fetch("<?php echo site_url('master/item/c_it3mun1t/deleteData/?id='); ?>"+docNum+'&reason='+result.value)
